...
export function Rank ({ modalInfo , setModalInfo }: RankProps) {
    ...
    return (
        <Model isOpen={modalInfo} setIsOpen={setModalInfo}>
            <ModelHeader title ={"Last Games"} onClose={() => setModalInfo(false)}/>
                        ...
                        <tbody>
                            {lastGames.map((lastGame,index) => (
                                <tr className="text-center" key={index}>
                                    <td className="p-2">{new Date(lastGame.timestamp).toLocaleString()}
                                    </td>
                                    <td className="p-2">{lastGame.score}</td>
                                    <td className="p-2">{getCategoryName(lastGame.category)}</td>
                                    <td className="p-2">
                                        {capitalizeFirstLetter(lastGame.difficulty)}</td>
                                </tr>
                            ))}
